...
hubspot_mql as (
    ...
    where
        hs_lead_score >= {{ var('mql_threshold_naive_hubspot') }}
        and not is_internal
)
...
select
    ...
    v.divergence_reason,
    v.fix_owner
from pipeline_variants as v
cross join canon_pipeline as c
...
select
    ...
    h.value,
    m.value,
    round(h.value - m.value, 2),
    round((h.value - m.value) / nullif(m.value, 0), 4),
    'HubSpot marks MQL at lead score >= 40; the canonical threshold is >= 50, so HubSpot reports ~15% more MQLs.',
    'Marketing Ops'
from hubspot_mql as h
cross join canon_mql as m
