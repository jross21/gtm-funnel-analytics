...
with sources as (
    ...
    union all
    select 'raw_sf_opportunity_stage_history', max(_synced_at) from {{ ref('raw_sf_opportunity_stage_history') }}
    union all
    select 'raw_sf_accounts', max(_synced_at) from {{ ref('raw_sf_accounts') }}
    union all
    select 'raw_sf_users', max(_synced_at) from {{ ref('raw_sf_users') }}
    union all
    select 'raw_sf_activities', max(_synced_at) from {{ ref('raw_sf_activities') }}
    union all
    select 'raw_hs_contacts', max(_synced_at) from {{ ref('raw_hs_contacts') }}
    union all
    select 'raw_hs_marketing_events', max(_synced_at) from {{ ref('raw_hs_marketing_events') }}
    union all
    select 'raw_hs_lead_score_history', max(_synced_at) from {{ ref('raw_hs_lead_score_history') }}
),
...
select
    ...
    case
        when age_hours > {{ error_hours }} then 'Stale'
        when age_hours > {{ warn_hours }} then 'Warn'
        else 'Fresh'
